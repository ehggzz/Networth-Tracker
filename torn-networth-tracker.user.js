...
(async () => {
  ...
  function key(){ return apiKey && apiKey !== PDA_KEY ? String(apiKey).trim() : ""; }

  async function getKey(){
    try { if(typeof PDA_storage!=="undefined"){const k=String((await PDA_storage.get(KEY_STORE,""))||"").trim();if(k)return k;} } catch(e){}
  ...
  function components(nw){
    ...
    add("Bank",nw?.money?.bank ?? nw?.bank);
  ...
  async function refresh(){
    ...
    try{
      ...
      const m=moneyR.money||{};
      const nw=nwR.networth||{};
      const snapshot={timestamp:Date.now(),cash:num(m.wallet),networth:totalNW(nw),components:components(nw)};
      const d=await load();
      d.current=snapshot;
      d.snapshots.push(snapshot);
      d.statsCurrent=stats;
      d.statsSnapshots.push({timestamp:Date.now(),stats});
      ...
      d.apiStatus="Connected"; d.apiError=null; d.lastChecked=Date.now();
      await save(d); render(d);
    }catch(e){
  ...
  function deltaStats(d){
    const base=d.statsSnapshots.find(x=>x.timestamp>=dayStart());
    const cur=d.statsCurrent||{};
    const delta={};
  ...
  function styles(){
    ...
    const s=document.createElement("style");s.id=ROOT+"-style";s.textContent=`
      ...
      #${ROOT} .nwt-arrow{float:right;opacity:.7} #${ROOT} .nwt-panel{display:none;margin-top:2px;padding:12px;border-radius:0 0 4px 4px;background:rgba(24,24,24,.96);font-size:12px;line-height:1.45}
